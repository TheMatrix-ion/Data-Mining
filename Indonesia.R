...
df <- read.csv(file = "Indonesia_data.csv",header = T)
df <- select(df, -c(Year_Month, Event))
...
reg2 = lm(Deaths ~ PC1 + PC2 + PC3 + PC6 , data = df_regression)
summary(reg2)
...
# Classification————Yaojie Zhang
# Load data
data <- read.csv("Indonesia_data.csv")
...
data$Event <- factor(data$Event, levels = c("CLIMATE CHANGE", "CONFLICT", "DROUGHT", 
                                            "EARTHQUAKE", "EARTHQUAKE AND TSUNAMI","TSUNAMI",
                                            "ERUPTION", "FIRE", "FLOODS","LAINNYA",
                                            "FOREST FIRE", "LANDSLIDES","STRONG WIND",
                                            "SURGE","TERRORISM","TRANSPORTATION ACCIDENT"))
