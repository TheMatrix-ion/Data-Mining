...
df <- read.csv(file = "Nepal_data.csv",header = T)
...
reg1 = lm(Deaths ~ PC1 + PC2 + PC3 + PC4 + PC5 + PC6 + PC7 + PC8, data = df_regression)
summary(reg1)
...
data$Event <- factor(data$Event, levels = c("ACCIDENT", "AVALANCHE", "BIOLOGICAL", 
                                            "BOAT CAPSIZE", "COLD WAVE", "DROUGHT", 
                                            "EARTHQUAKE", "EPIDEMIC","EXPLOSION",
                                            "FAMINE","FIRE","FLOOD","FOREST FIRE",
                                            "FROST","HAIL STORM","HEAT WAVE",
                                            "LANDSLIDE","LEAK","OTHER","PANIC",
                                            "PLAGUE","POLLUTION","RAINS","STORM",
                                            "SEDIMENTATION","SNOW STORM","STRONG WIND",
                                            "STRUCT.COLLAPSE","THUNDERSTORM"))
